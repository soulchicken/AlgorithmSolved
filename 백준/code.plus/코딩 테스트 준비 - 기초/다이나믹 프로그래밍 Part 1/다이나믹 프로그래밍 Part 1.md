...
5. 11052_카드 구매하기
    ...
    > - 과정
    >   1. 1개를 구매하는 최댓값(C1)은 P1
    >   2. 2개를 구매하는 최댓값(C2)는 max(P2,C1 + C1)
    >   3. 3개를 구매하는 최댓값(C3)는 max(P3, C2 + C1, C1 + C2)
    >   4. k개를 구매하는 최댓값(Ck)는 max(Pk, Ci + Cj (i+j = k))
...
6. 카드 구매하기 2
    ...
    > - 과정
    >   1. 1개를 구매하는 최솟값(C1)은 P1
    >   2. 2개를 구매하는 최솟값(C2)는 min(P2,C1 + C1)
    >   3. 3개를 구매하는 최솟값(C3)는 min(P3, C2 + C1, C1 + C2)
    >   4. k개를 구매하는 최솟값(Ck)는 min(Pk, Ci + Cj (i+j = k))
...
12. 1912_연속합
    ...
    > - 과정
    >   1. li 리스트에 입력 받는다.
    >   2. li[1]이 0보다 크다면 li[1] += li[0]
    >   3. li[2]가 0보다 크다면 li[2] += li[1]
    >   4. li[i]가 0보다 크다면 li[i] += li[i-1]
...
14. 퇴사
    ...
    > - 입력 : 첫째 줄에 N (1 ≤ N ≤ 15)이 주어진다. 둘째 줄부터 N개의 줄에 Ti와 Pi가 공백으로 구분되어서 주어지며, 1일부터 N일까지 순서대로 주어진다. (1 ≤ Ti ≤ 5, 1 ≤ Pi ≤ 1,000)
    > - 출력 : 첫째 줄에 백준이가 얻을 수 있는 최대 이익을 출력한다.
    > - 과정
    >   1. 입력을 받는다 (T 리스트에 시간, P 리스트에 금액을 넣는다). 그리고 인덱스별로 최대 금액을 리스트로 만든다. 시작은 모든 금액이 -1이다.
    >   2. 큐에 `(1,0)`을 넣는다. 큐에 들어갈 `(index,pay)`는 해당 인덱스 위치에 이익을 뜻한다.
    >   3. while문에서 큐를 하나씩 pop한다. pop으로 나온 index 위치의 금액이 pay보다 작을 경우 pay로 값을 변경한다.
    >   4. 값 변경이 이뤄지면 큐에 `(index+1,pay) 과 (index + T[index],pay + P[index])`를 append한다.
    >   5. N+1일자 최대 금액을 출력한다.

15. 합분해

    > - 입력 : 0부터 N까지의 정수 K개를 더해서 그 합이 N이 되는 경우의 수를 구하는 프로그램을 작성하시오. 덧셈의 순서가 바뀐 경우는 다른 경우로 센다(1+2와 2+1은 서로 다른 경우). 또한 한 개의 수를 여러 번 쓸 수도 있다.
    >   첫째 줄에 두 정수 N(1 ≤ N ≤ 200), K(1 ≤ K ≤ 200)가 주어진다.
    > - 출력 : 첫째 줄에 답을 1,000,000,000으로 나눈 나머지를 출력한다.
    > - 과정
    >   1. 0을 1개, 2개, K개의 수로 사용한 경우의 수는 모두 1개
    >   2. 0부터 N까지 1개까지 사용한 경우의 수는 모두 1개
    >   3. 1을 2개의 수를 사용한 경우는 0을 1개의 수로 만든 경우의 수 + 0을 2개의 수로 사용한 경우의 수
    >   4. n을 count 개의 수의 합으로 만드는 경우의 수는 (n - 1)을 count개의 수로 만드는 경우의 수와 n을 (count - 1)개의 수의 합으로 만든 경우의 수
